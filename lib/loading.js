  ...
  Object.keys(map).forEach(function (key) {
    ...
    if (Array.isArray(mod.extra)) {
      ...
      mod.extra.forEach(function (extra) {
        // avoid duplicating
        if (nextTarget[extra] !== undefined) {
          return;
        }

        // load extra
        var extraPath = path.join(base, extra);
        debug('loading extra: %s into target.%s.%s', extraPath, field, extra);
        self._load(nextTarget, extra, getMods(extraPath));
      });
    }
  });
};
